...
const PLANS: Plan[] = [
  {
    ...
    tagline: { ar: 'كتاب واحد مخصص', en: 'A single personalized book' },
    priceMultiplier: 1, // Anchor is 17 KD
    shipping: { ar: 'تطبق رسوم شحن', en: 'Shipping fees apply' },
    perks: [
      { label: { ar: 'كتاب واحد مخصص', en: '1 personalized storybook' }, available: true },
      { label: { ar: 'توليد ذكاء اصطناعي (قصة + رسم)', en: 'Full AI generation (story + illustrations)' }, available: true },
      { label: { ar: 'نسخة رقمية (PDF)', en: 'Soft copy (PDF download)' }, available: true },
      { label: { ar: 'نسخة مطبوعة (اختياري)', en: 'Printed copy (if selected)' }, available: true },
      { label: { ar: 'بدون التزام شهري', en: 'No commitment' }, available: true },
      { label: { ar: 'تنفيذ سريع', en: 'Fast turnaround (no cycle waits)' }, available: true },
      { label: { ar: 'ملكية كاملة للكتاب', en: 'Full ownership of the book' }, available: true },
      { label: { ar: 'إضافات اختيارية', en: 'Optional upgrades' }, available: true },
      { label: { ar: 'إعادة توليد', en: 'No regeneration' }, available: false },
      { label: { ar: 'أولوية في التنفيذ', en: 'No priority in queue' }, available: false },
      { label: { ar: 'استمرارية للمواضيع', en: 'No theme continuity' }, available: false },
      { label: { ar: 'مزايا الاشتراك', en: 'No subscription benefits' }, available: false },
    ]
  },
  {
    id: 'monthly',
    ...
    tagline: { ar: 'تجربة قصصية مستمرة آلياً', en: 'Continuous automated storytelling' },
    priceMultiplier: 14 / 17, // ~14 KD
    shipping: { ar: 'شحن مستمر', en: 'Continuous shipping' },
    perks: [
      { label: { ar: 'كتاب تلقائي كل شهر', en: '1 book per month (auto-generated)' }, available: true },
      { label: { ar: 'تجربة قصصية مستمرة', en: 'Continuous storytelling experience' }, available: true },
      { label: { ar: 'نسخة رقمية كل دورة', en: 'Soft copy every cycle' }, available: true },
      { label: { ar: 'نسخة مطبوعة إضافية', en: 'Optional print add-on' }, available: true },
      { label: { ar: 'تجربة مؤتمتة', en: 'Automated experience' }, available: true },
      { label: { ar: 'تدوير المواضيع', en: 'Theme rotation (no repeats)' }, available: true },
...
const CheckoutScreen: React.FC<CheckoutScreenProps> = ({ onProceedToPayment, onBack, language, storyData, currency }) => {
  ...
  const [basePrice, setBasePrice] = useState<number>(17.000); // Anchor price for Standard One-Time
  ...
  React.useEffect(() => {
    // We'll keep the size-based logic if available, but anchor it to 17.000 for standard
