...
const ThemeCard: React.FC<ThemeCardProps> = ({ emoji, title, description, isSelected, onClick }) => (
    <button
        onClick={onClick}
        className={`relative p-5 text-center rounded-2xl transition-all duration-300 h-full flex flex-col items-center justify-start group ${
            isSelected
...
const ThemeScreen: React.FC<ThemeScreenProps> = ({ onNext, onBack, storyData, language }) => {
  ...
  useEffect(() => {
    setThemes(adminService.getThemes());
  }, []);
