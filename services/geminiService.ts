...
export async function generateMethod4Image(prompt: string, stylePrompt: string, referenceBase64: string, characterDescription: string, age: string, seed?: number): Promise<{ imageBase64: string; fullPrompt: string }> {
    // This uses the Proxy to Imagen 3/4 via generateImagenImage
    const finalPrompt = `
     subject_description: ${characterDescription}
     subject_age_lock: ${age}
     style_instruction: ${stylePrompt}
     ${prompt}
     `;

    const { imageBase64 } = await generateImagenImage(finalPrompt, "16:9", 1);
